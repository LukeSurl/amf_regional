...
C  write messages
	...
	IF ( STATUS .EQ. LIDORT_SERIOUS ) THEN
	  IF ( TRACE .EQ. ' ' ) THEN
	  ...
	  ENDIF
	ENDIF
C  turn of  error initialization flag

	LIDORT_ERROR_INIT = .FALSE.

C  finish
C This is way too complicated. Just write to screen
        print*,"LIDORT error: MESSAGE"
        print*, MESSAGE
        print*,"LIDORT error: TRACE"
        print*, TRACE
        call flush(5)
...
C  input/output arguments
	...
	DOUBLE PRECISION  AAD(MAXSTRM,MAXSTRM), WKD(MAXSTRM2),
     &                    EVALD(MAXSTRM), EVECD(MAXSTRM,MAXSTRM)
